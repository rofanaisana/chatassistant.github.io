    ...
    card.innerHTML = `
      ...
      <div class="form-group">
        ...
        <select class="form-select" onchange="updateCharInput(${char.id}, 'side', this.value)">
          <option value="left" ${char.side === 'left' ? 'selected' : ''}>← 좌측 (상대방)</option>
          <option value="right" ${char.side === 'right' ? 'selected' : ''}>→ 우측 (나)</option>
        </select>
      </div>
    `;
...
function renderMessages() {
  ...
  state.messages.forEach(msg => {
    ...
    if (msg.type === 'scene') { badgeClass = 'badge-scene'; badgeText = '장면설명'; }
    ...
    const placeholder = msg.type === 'action' ? '(행동 묘사를 입력하세요)' :
                        msg.type === 'scene' ? '장면/장소 설명을 입력하세요' :
                        '대사를 입력하세요';
...
function buildMessageHTML(msg, actionMode) {
  ...
  if (msg.type === 'action') {
    ...
    return `<div class="action-block">${esc(msg.text || '(지문)')}</div>`;
  }
